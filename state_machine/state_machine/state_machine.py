...
from std_msgs.msg import String, Bool # added(09.16)
from pathlib import Path # added(09.16)
try:
...
class StateMachine(Node):
    ...
    def _get_or_declare_str(self, name: str, default: str) -> str:
        if self.has_parameter(name):
            try:
                # rclpy >= Humble에선 .value로 바로 꺼내는 게 보통 됩니다.
    ...
    def __init__(self):
        ...
        # === [ctrl-sectors] enable via SetParameters & sector state ===
        self.ctrl_sectors = []         # [{'start':int,'end':int,'pp':bool,'tln':bool}, ...]
        self._cur_sector_idx = None
        ...
        # SetParameters 서비스 클라이언트 준비
        self.pp_param_cli  = self.create_client(SetParameters, f'{self.pp_node_name}/set_parameters')
        self.tln_param_cli = self.create_client(SetParameters, f'{self.tln_node_name}/set_parameters')

        # 서비스 준비 대기 (한 번만)
        if not self.pp_param_cli.wait_for_service(timeout_sec=2.0):
            self.get_logger().warn(f'[CTRL-SECTORS] {self.pp_node_name}/set_parameters not available yet')
        if not self.tln_param_cli.wait_for_service(timeout_sec=2.0):
            self.get_logger().warn(f'[CTRL-SECTORS] {self.tln_node_name}/set_parameters not available yet')

        # 마지막으로 전송한 enable 상태 캐시 (중복 호출 방지)
        self._last_pp_enable  = None
        self._last_tln_enable = None

        # 섹터 YAML 로드 (파일 읽기)
        self._load_ctrl_sectors_yaml()
        # === [/ctrl-sectors] ===
    ...
    # === [ctrl-sectors] helpers =================================================

    # === [ctrl-sectors] helpers =================================================
    def _load_ctrl_sectors_yaml(self):
        """
        ctrl_sectors.yaml을 (1) 소스 트리(src) 우선, 없으면 (2) install 경로에서 로드한다.
        최종 형태는 self.ctrl_sectors = [{'start':..,'end':..,'pp':..,'tln':..}, ...]
        """
        # map_name 확보
        try:
        ...
        # - install:  <ws>/install/stack_master/maps/<map_name>/ctrl_sectors.yaml
        try:
            sm_share = Path(get_package_share_directory('stack_master'))  # .../install/stack_master/share/stack_master
        except Exception as e:
            self.get_logger().error(f'[CTRL-SECTORS] cannot get share dir: {e}')
            self.ctrl_sectors = []
            return

        # src 우선 경로
        src_base      = sm_share.parents[3] / 'stack_master' / 'maps'   # .../race_stack/stack_master/maps
        install_base  = sm_share.parent.parent.parent / 'stack_master' / 'maps'  # .../install/stack_master/maps

        src_yaml      = src_base     / map_name / 'ctrl_sectors.yaml'
        install_yaml  = install_base / map_name / 'ctrl_sectors.yaml'

        # 우선순위: src -> install
        if src_yaml.exists():
            yaml_path = src_yaml
        elif install_yaml.exists():
            yaml_path = install_yaml
        else:
            self.get_logger().warn(f'[CTRL-SECTORS] file not found in both:\n  - {src_yaml}\n  - {install_yaml}')
            self.ctrl_sectors = []
            return
        ...
        try:
            ...
            params = data['sector_runtime_ctrl_switcher']['ros__parameters']
            n = int(params.get('n_sectors', 0))

            sectors = []
            for i in range(n):
                sec = params.get(f'Sector{i}')
                if not sec:
                    self.get_logger().warn(f'[CTRL-SECTORS] missing Sector{i}; skip')
                    continue
                start = int(sec['start'])
                end   = int(sec['end'])
                ctrls = sec.get('controllers', {})
                pp    = bool(ctrls.get('pp', False))
                tln   = bool(ctrls.get('tln', False))
                sectors.append({'start': start, 'end': end, 'pp': pp, 'tln': tln})

            self.ctrl_sectors = sectors
            self.get_logger().info(f'[CTRL-SECTORS] loaded {len(sectors)} sectors from {yaml_path}')
        except Exception as e:
            self.get_logger().error(f'[CTRL-SECTORS] parse error: {e}')
            self.ctrl_sectors = []
    # === [/ctrl-sectors] ========================================================
    def _current_wp_index(self):
        """
        cur_s(미터) -> 웨이포인트 인덱스(정수). 경계 클램프.
        glb_wpnts_scaled_cb에서 계산되는 간격 이름이 혼재하므로 둘 다 대응.
        """
        if self.cur_s is None:
            return None
        # waypoints_dist(초기값 0.1) 혹은 wpnt_dist(콜백에서 설정)를 사용
        dist = getattr(self, 'waypoints_dist', None) or getattr(self, 'wpnt_dist', None)
        if not dist:
            return None
        idx = int(round(self.cur_s / dist))
        idx = max(0, min(self.gb_max_idx, idx))
        return idx

    def _find_ctrl_sector_idx(self, wp_idx):
        if wp_idx is None or not self.ctrl_sectors:
            return None
        for i, sec in enumerate(self.ctrl_sectors):
            if sec['start'] <= wp_idx <= sec['end']:
                return i
        return None

    def _set_node_param_bool(self, client, name: str, value: bool):
        """
        SetParameters 클라이언트로 bool 파라미터 1개를 비동기 전송.
        서비스가 아직 없으면 그냥 건너뛰고 다음 루프에서 재시도.
        """
        if client is None:
            return
        ...
        try:
            ...
            client.call_async(req)  # 비동기: 결과 기다릴 필요 없음
        except Exception as e:
            self.get_logger().warn(f'[CTRL-SECTORS] set param failed ({name}={value}): {e}')

    def _set_ctrl_enables(self, pp_enable: bool, tln_enable: bool):
        """
        캐시와 비교해서 변경이 있을 때만 실제 파라미터 세팅 호출.
        파라미터 이름은 각 컨트롤러 노드가 declare_parameter("drive_publish_enabled")로 선언해 둬야 함.
        """
        if self._last_pp_enable is None or self._last_pp_enable != pp_enable:
            self._set_node_param_bool(self.pp_param_cli,  'drive_publish_enabled', bool(pp_enable))
            self._last_pp_enable = bool(pp_enable)

        if self._last_tln_enable is None or self._last_tln_enable != tln_enable:
            self._set_node_param_bool(self.tln_param_cli, 'drive_publish_enabled', bool(tln_enable))
            self._last_tln_enable = bool(tln_enable)

    def _publish_ctrl_enables_if_changed(self):
        """
        현재 위치 기반 섹터가 바뀔 때만 각 컨트롤러의 drive_publish_enabled 파라미터를 업데이트.
        """
        if not self.ctrl_sectors:
            return
        wp_idx = self._current_wp_index()
        sec_i = self._find_ctrl_sector_idx(wp_idx)
        if sec_i is None or sec_i == self._cur_sector_idx:
            return

        self._cur_sector_idx = sec_i
        sec = self.ctrl_sectors[sec_i]
        self.get_logger().info(
            f'[CTRL-SECTOR] #{sec_i}: {sec["start"]}-{sec["end"]} | pp={sec["pp"]}, tln={sec["tln"]}'
        )
        self._set_ctrl_enables(sec['pp'], sec['tln'])
    ...
    def main_loop_callback(self):
        ...
        self._publish_ctrl_enables_if_changed()
