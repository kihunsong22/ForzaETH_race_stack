...
class PP_Controller:
    ...
    def main_loop(self, state, position_in_map, waypoint_array_in_map, speed_now, opponent, position_in_map_frenet, acc_now, track_length,opp_as=0.0, opp_ad=0.0):
        ...
        if (L1_point is not None) and (np.asarray(L1_point).size != 0) and np.all(np.isfinite(L1_point)):
            steering_angle = self.calc_steering_angle(L1_point, L1_distance, yaw, lat_e_norm, v)
        else: 
            raise Exception("L1_point is None")
        
        # ================= Predictive Safety Logic  =================
        if self.opponent is not None and len(self.opponent) >= 6 and self.position_in_map_frenet is not None:
            ...
            is_overtaking = abs(steering_angle) > 0.1 or dist_d > 0.3
                        
            # 5. Set Dynamic Thresholds
            safe_dist_s = 0.8 if is_overtaking else 1.5  # Allow closer if overtaking
            safe_dist_d = 0.3

            #6. Dual Mode Safety Logic
            if is_overtaking:
                # [Overtaking] 
                # Ignore safety distance for acceleration! Only stop if collision is imminent.
                # (Critical: < 0.6m s-dist AND < 0.3m d-dist)
                if 0 < dist_s < 0.6 and dist_d < 0.3: 
                    self.logger_warn(f"[Safety] CRITICAL! Emergency Stop. Dist: {dist_s:.2f}")
                    speed = 0.0
                    acceleration = -5.0
            else:
                # [Following] 
                # Maintain safe distance (1.2m)
                if 0 < dist_s < 1.2 and dist_d < 0.3:
    ...
    def calc_L1_point(self, lateral_error):
        ...
        if len(self.waypoint_array_in_map[self.idx_nearest_waypoint:]) > 2:
            ...
            # if self.max_curvature < self.curvature_waypoints:
            #     self.max_curvature = self.curvature_waypoints
            #     self.logger_info(f"[PP Controller] max curvature mean updated to {self.max_curvature}")
        ...
        # self.logger_info(f"[PP Controller]: {self.t_clip_min}, {np.sqrt(2)*lateral_error}")
        lower_bound = max(self.t_clip_min, np.sqrt(2)*lateral_error)
        # self.logger_info(f"[PP Controller] L1 distance before clip: {L1_distance}")
        L1_distance = np.clip(L1_distance, lower_bound, self.t_clip_max)
        # self.logger_info(f"[PP Controller] L1 distance after clip: {L1_distance}")
    ...
    def speed_adjust_lat_err(self, global_speed, lat_e_norm):
        ...
        # curv = np.clip(2*(np.mean(self.curvature_waypoints)/0.28) - 2, a_min = 0, a_max = 1) # 0.28 ca. max curvature mean
        curv = np.clip(self.curvature_waypoints/0.28, 0, 1) # 0.28 ca. max curvature mean
        
        scale = (1 - lat_e_coeff + lat_e_coeff*np.exp(-lat_e_norm*curv))

        self.logger_info(f"[PP Controller]: lat_e_norm {lat_e_norm}, curv {curv}, scale {scale}")
    ...
    #     self.logger_info(f"[PP Controller]: heading error {heading_error*180/np.pi} degrees")

    #     # if heading_error < np.pi/9: # 20 degrees error is okay
    #     if heading_error < np.pi/15: # 12 degrees error is okay
    #         return speed_command
    #     elif heading_error < np.pi/2: 
    #         # scaler = 1 - 0.5* heading_error/(np.pi/2) # scale linearly to 0.5x
    #         scaler = 1 - 0.65* heading_error/(np.pi/2) # scale linearly to 0.5x
    #     else:
    #         scaler = 0.5
    ...
    def speed_adjust_heading(self, speed_command):
        """
        헤딩 오차가 클수록 속도 감속.
        12° 이하는 무시, 90°에서 최소 스케일까지 선형 감속.
        """
        if self.waypoint_array_in_map is None or len(self.waypoint_array_in_map) == 0:
            return speed_command
        if self.position_in_map is None:
            return speed_command

        # 최근접 웨이포인트 인덱스를 내부에서 계산 (순서 의존성 제거)
        idx = int(self.nearest_waypoint(
            self.position_in_map[0, :2],
        ...
        # 래핑된 각도 차이 [-pi, pi]
        dpsi = np.arctan2(np.sin(heading - map_heading), np.cos(heading - map_heading))
        heading_error = abs(dpsi)  # scalar

        # 임계값
        ok_thresh   = np.pi / 15   # ≈ 12 deg
        hard_thresh = np.pi / 2    # 90 deg
        min_scale   = 0.5          # 최소 50%

        if heading_error <= ok_thresh:
            scale = 1.0
        elif heading_error < hard_thresh:
            # 12°~90° 구간에서 1.0 → min_scale 선형 감소
            scale = min_scale + (1.0 - min_scale) * (1.0 - (heading_error - ok_thresh) / (hard_thresh - ok_thresh))
        else:
            scale = min_scale

        self.logger_info(f"[PP Controller] heading error={heading_error*180/np.pi:.1f} deg, speed scale={scale:.2f}")
